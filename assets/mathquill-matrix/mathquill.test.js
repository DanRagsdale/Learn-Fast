        ...
        var EditableField = /** @class */ (function (_super) {
            ...
            EditableField.prototype.cmd = function (cmd) {
                ...
                if (/^\\[a-z]+$/i.test(cmd) && !cursor.isTooDeep()) {
                    cmd = cmd.slice(1);
                    var klass = LatexCmds[cmd] || EnvironmentCmds[cmd];
                    var node;
                    if (klass) {
            ...
            EditableField.prototype.matrixCmd = function (cmd) {
                var args = [];
                ...
                if (cursor.parent instanceof MatrixCell &&
                    ...
                    var blockindex = cursor.parent.parent.blocks.indexOf(cursor.parent);
                    if (cmd === 'addColumn') {
                    ...
                    var rows = args[1];
                    var cols = args[2];
                    if (EnvironmentCmds.hasOwnProperty(envtype) &&
                        typeof rows === 'number' &&
                        typeof cols === 'number') {
                        var latex = '\\begin{' + envtype + '}';
                        var row = '';
                        ...
                        if (cursor[L] instanceof Matrix) {
                            ...
                            if (cursorL.blocks) {
                                var firstcell = cursorL.blocks[0];
    ...
    */
    ...
    var SVG_SYMBOLS = {
        ...
        '[': {
            width: '.55em',
            html: function () {
                return h('svg', { preserveAspectRatio: 'none', viewBox: '0 0 100 100' }, [
                    h('path', {
    ...
    LatexCmds.begin = /** @class */ (function (_super) {
        ...
        begin.prototype.parser = function () {
            var string = Parser.string;
            var regex = Parser.regex;
            return string('{')
                .then(regex(/^[a-z]+/i))
    ...
    var Environment = /** @class */ (function (_super) {
        ...
        Environment.prototype.wrappers = function () {
            return [
    ...
    var Matrix = /** @class */ (function (_super) {
        ...
        function Matrix(leftParentheses, rightParentheses, environment) {
            var _this_1 = _super.call(this) || this;
            _this_1.delimiters = {
                column: '&',
                row: '\\\\',
            };
            _this_1.parentheses = { left: '[', right: ']' };
            _this_1.blocks = [];
            _this_1.rowSize = 0;
        ...
        Matrix.prototype.html = function () {
            var cells = [], row = -1;
            var leftch = this.parentheses.left;
            var leftBracketSymbol = SVG_SYMBOLS[leftch] || { width: '0' };
            var rightch = this.parentheses.right;
            var rightBracketSymbol = SVG_SYMBOLS[rightch] || { width: '0' };
            this.domView = new DOMView(cells.length, function (blocks) {
                var rows = [];
                var tds = [];
        ...
        Matrix.prototype.getEntryPoint = function (dir, updown) {
            if (updown === 'up') {
                if (dir === L) {
        ...
        Matrix.prototype.atExitPoint = function (dir, cursor) {
        ...
        Matrix.prototype.moveTowards = function (dir, cursor, updown) {
        ...
        Matrix.prototype.relink = function () {
            ...
            for (var i = 0; i < blocks.length; i += 1) {
                ...
                cell[R] = (_d = blocks[i + 1]) !== null && _d !== void 0 ? _d : 0;
                cell[L] = (_e = blocks[i - 1]) !== null && _e !== void 0 ? _e : 0;
            ...
            this.setEnds((_c = {}, _c[L] = blocks[0], _c[R] = blocks[blocks.length - 1], _c));
        };
        Matrix.prototype.updateMathspeakTemplate = function () {
            ...
            for (var i = 0; i < this.blocks.length; i++) {
                var currow = Math.floor(i / this.rowSize) + 1;
                var curcol = (i % this.rowSize) + 1;
                var thistext = '';
                if (curcol === 1) {
                ...
                if (i == 0) {
                    newTemplate.push('Start ' + rowcnt + ' By ' + colcnt + ' Matrix ' + thistext);
                }
                else {
        ...
        Matrix.prototype.autocorrect = function () {
            var lengths = [], rows = [];
            var blocks = this.blocks;
        ...
        Matrix.prototype.deleteCell = function (currentCell) {
            var rows = [], columns = [], myRow = [], myColumn = [];
            var blocks = this.blocks, row = -1, column = 0;
            ...
            if (isEmpty(myRow) && myColumn.length > 1) {
                ...
                */
                var tofix = this.domFrag().oneElement().querySelectorAll('tr');
        ...
        Matrix.prototype.deleteColumn = function (basecellindex, cursor) {
        ...
        Matrix.prototype.deleteRow = function (basecellindex, cursor) {
            var rowSize = this.rowSize;
            var rownum = Math.floor(basecellindex / rowSize);
        ...
        Matrix.prototype.addColumn = function (basecellindex, dir) {
            var rowSize = this.rowSize;
            var colnum = basecellindex % rowSize;
            var row = 0;
            var self = this;
            ...
            while (colnum < this.blocks.length) {
                var newcell = new MatrixCell(row, self);
                newcell.setDOM(domFrag(h('td', { class: 'mq-empty' }, []))
                    .insDirOf(dir, this.blocks[colnum].domFrag())
                    .oneElement());
        ...
        Matrix.prototype.addRow = function (basecellindex, dir) {
            var rowSize = this.rowSize;
            var self = this;
            if (dir !== 1 && dir !== -1) {
                dir = 1;
            ...
            for (var i = 0; i < rowSize; i++) {
                var newcell = new MatrixCell(rownum, self);
                newcell.setDOM(domFrag(h('td', { class: 'mq-empty' }, []))
        ...
        Matrix.prototype.backspace = function (cell, dir, cursor, finalDeleteCallback) {
    ...
    EnvironmentCmds.matrix = function () { return new Matrix('', '', 'matrix'); };
    EnvironmentCmds.pmatrix = function () { return new Matrix('(', ')', 'pmatrix'); };
    EnvironmentCmds.bmatrix = function () { return new Matrix('[', ']', 'bmatrix'); };
    EnvironmentCmds.Bmatrix = function () { return new Matrix('{', '}', 'Bmatrix'); };
    EnvironmentCmds.vmatrix = function () { return new Matrix('|', '|', 'vmatrix'); };
    EnvironmentCmds.Vmatrix = function () { return new Matrix('&#8741;', '&#8741;', 'Vmatrix'); };
        ...
        function MatrixCell(row, parent, replaces) {
            ...
            if (replaces) {
                for (var i = 0; i < replaces.length; i++) {
                    replaces[i].children().adopt(_this_1, _this_1.ends[R], 0);
                }
            }
            return _this_1;
        }
        MatrixCell.prototype.deleteOutOf = function (dir, cursor) {
            ...
            if (this.parent instanceof Matrix) {
                this.parent.backspace(this, dir, cursor, function () {
                    return _super.prototype.deleteOutOf.call(_this_1, dir, cursor);
                });
            }
        };
        MatrixCell.prototype.moveOutOf = function (dir, cursor, updown) {
            var atExitPoint = updown &&
    ...
    suite('latex', function () {
        ...
        test('matrices', function () {
            ...
            assertParsesLatex('\\begin{bmatrix}x&y&z&123&x^{2}\\\\23&s&\\theta &1&x\\\\e&h&a&1&y\\end{bmatrix}');
            // Adds missing cells
    ...
    suite('matrix', function () {
        ...
        suite('Matrices', function () {
            ...
            test('brackets are scaled immediately when height is changed', function () {
                ...
                assert.ok(bracketHeight() > height, 'matrix bracket height should be increased when height is changed');
            });
        });
    });
    ...
    suite('DOMFragment', function () {
        ...
        var _loop_5 = function (name, apply) {
            suite(name, function () {
